...
public class RegistryHelper {
	...
	public void sendC2SPacket(CustomPayload packet) {
		ClientProxy.Holder.INSTANCE.sendC2SPacket(packet);
	}
	...
	public Supplier<EntityType<PrimedLTNT>> registerTNTEntity(String entityRegistry, String registryName, PrimedTNTEffect effect, float size, boolean fireImmune){
		Identifier id = Identifier.of(entityRegistry, registryName);
		RegistryKey<EntityType<?>> key = RegistryKey.of(Registries.ENTITY_TYPE.getKey(), id);
		if(fireImmune) {
			EntityType<PrimedLTNT> rtype = Registry.register(Registries.ENTITY_TYPE, id, EntityType.Builder.<PrimedLTNT>create((EntityType<PrimedLTNT> type, World level) -> new PrimedLTNT(type, level, effect), SpawnGroup.MISC)/*.setShouldReceiveVelocityUpdates(true)*/.maxTrackingRange(64).makeFireImmune().dimensions(size, size).build(key));
			return () -> rtype;
		}
		else {
			EntityType<PrimedLTNT> rtype = Registry.register(Registries.ENTITY_TYPE, id, EntityType.Builder.<PrimedLTNT>create((EntityType<PrimedLTNT> type, World level) -> new PrimedLTNT(type, level, effect), SpawnGroup.MISC)/*.setShouldReceiveVelocityUpdates(true)*/.maxTrackingRange(64).dimensions(size, size).build(key));
			return () -> rtype;
		}
	}
	...
	public Supplier<EntityType<LTNTMinecart>> registerTNTMinecart(String entityRegistry, String registryName, Supplier<EntityType<PrimedLTNT>> TNT, Supplier<Supplier<LTNTMinecartItem>> pickItem, boolean explodesInstantly){
		Identifier id = Identifier.of(entityRegistry, registryName);
		RegistryKey<EntityType<?>> key = RegistryKey.of(Registries.ENTITY_TYPE.getKey(), id);
		EntityType<LTNTMinecart> rtype = Registry.register(Registries.ENTITY_TYPE, id, EntityType.Builder.<LTNTMinecart>create((EntityType<LTNTMinecart> type, World level) -> new LTNTMinecart(type, level, TNT, pickItem, explodesInstantly), SpawnGroup.MISC)/*.setShouldReceiveVelocityUpdates(true)*/.maxTrackingRange(64).dimensions(0.98f, 0.7f).build(key));
		return () -> rtype;
	}
	...
	public Supplier<EntityType<LExplosiveProjectile>> registerExplosiveProjectile(String entityRegistry, String registryName, PrimedTNTEffect effect, float size, boolean fireImmune){
		...
		RegistryKey<EntityType<?>> key = RegistryKey.of(Registries.ENTITY_TYPE.getKey(), id);
		if(fireImmune) {
			EntityType<LExplosiveProjectile> rtype = Registry.register(Registries.ENTITY_TYPE, id, EntityType.Builder.<LExplosiveProjectile>create((EntityType<LExplosiveProjectile> type, World level) -> new LExplosiveProjectile(type, level, effect), SpawnGroup.MISC)/*.setShouldReceiveVelocityUpdates(true)*/.maxTrackingRange(64).makeFireImmune().dimensions(size, size).build(key));
			return () -> rtype;
		}
		else {
			EntityType<LExplosiveProjectile> rtype = Registry.register(Registries.ENTITY_TYPE, id, EntityType.Builder.<LExplosiveProjectile>create((EntityType<LExplosiveProjectile> type, World level) -> new LExplosiveProjectile(type, level, effect), SpawnGroup.MISC)/*.setShouldReceiveVelocityUpdates(true)*/.maxTrackingRange(64).dimensions(size, size).build(key));
