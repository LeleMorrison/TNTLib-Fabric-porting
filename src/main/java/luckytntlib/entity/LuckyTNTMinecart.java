...
public class LuckyTNTMinecart extends LTNTMinecart{
	...
	@Override
	public void fuse() {
		LTNTMinecart minecart = minecarts.get(new Random().nextInt(minecarts.size())).get().createMinecart(getEntityWorld(), getX(), getY(), getZ(), placer);
		minecart.setYaw(getYaw());
		minecart.setVelocity(getVelocity());
		getEntityWorld().spawnEntity(minecart);
		minecart.fuse();
		discard();
	}
}
