...
public class LTNTMinecart extends MinecartEntity implements IExplosiveEntity{
	...
	public LTNTMinecart(EntityType<LTNTMinecart> type, World level, Supplier<EntityType<PrimedLTNT>> TNT, Supplier<Supplier<LTNTMinecartItem>> pickItem, boolean explodeInstantly) {
		super(type, level);
		if(TNT != null) {
			PrimedLTNT tnt = TNT.get().create(level, SpawnReason.TRIGGERED);
	...
	public boolean damage(DamageSource source, float amount) {
		if (!getEntityWorld().isClient() && !isRemoved()) {
	...
	public void killAndDropSelf(ServerWorld world, DamageSource source) {
		...
		if (!source.isOf(DamageTypes.ON_FIRE) && !source.isOf(DamageTypes.EXPLOSION) && !(speed >= 0.01f)) {
			...
			if(getTNTFuse() < 0) {
				if(explodesInstantly()) {
					fuse();
					setTNTFuse(getEffect().getDefaultFuse(this) / 4 + world.random.nextInt(getEffect().getDefaultFuse(this)) / 4);
				}
				else {
					fuse();
				}
	...
	public void fuse() {
		...
		getEntityWorld().playSound(null, new BlockPos((int)getLerpedPos(1).x, (int)getLerpedPos(1).y, (int)getLerpedPos(1).z), SoundEvents.ENTITY_TNT_PRIMED, getSoundCategory(), 1f, 1f);
	}	@Override
	...
	public void readCustomDataFromNbt(NbtCompound tag) {
		tag.getInt("placerID").ifPresent(id -> {
