...
/**
 * ImprovedExplosion is a wrapper/helper for Minecraft's {@link Explosion}.
...
public class ImprovedExplosion {
	...
	private final java.util.Map<PlayerEntity, Vec3d> affectedPlayers = new java.util.HashMap<>();
	...
	public ImprovedExplosion(World level, @Nullable Entity explodingEntity, @Nullable DamageSource source, double x, double y, double z, int size) {
		...
		this.posZ = z;
		this.size = size;
		this.damageSource = source == null ? level.getDamageSources().explosion((Explosion)null) : source;
		this.damageCalculator = explodingEntity == null ? new ExplosionBehavior() : new EntityExplosionBehavior(explodingEntity);
	}
	...
	public void doBlockExplosion(float xzStrength, float yStrength, float resistanceImpact, float randomVecLength, boolean fire, boolean isStrongExplosion) {			
					...
					if (((int) distance == size && LuckyTNTLibConfigValues.PERFORMANT_EXPLOSION.get()) || (!LuckyTNTLibConfigValues.PERFORMANT_EXPLOSION.get() && (offX == -size || offX == size || offY == -size || offY == size || offZ == -size || offZ == size))) {
						...
						for (float vecStep = 0; vecStep < vecLength; vecStep += LuckyTNTLibConfigValues.EXPLOSION_PERFORMANCE_FACTOR.get() * 1.5f - 0.225f) {
							...
							if (!(isStrongExplosion && !fluidState.isEmpty())) {
								Optional<Float> explosionResistance = damageCalculator.getBlastResistance(null, level, pos, blockState, fluidState);
								if (explosionResistance.isPresent()) {
									vecLength -= (explosionResistance.get() + 0.3f) * 0.3f * resistanceImpact;
								}
								if (vecLength > 0 && damageCalculator.canDestroyBlock(null, level, pos, blockState, vecLength) && !blockState.isAir()) {
		...
		for(int intPos : blocks) {
			...
			if (level instanceof net.minecraft.server.world.ServerWorld serverWorld) {
				level.getBlockState(pos).getBlock().onDestroyedByExplosion(serverWorld, pos, null);
			}
		...
		if(fire) {
			for(int intPos : blocks) {
				BlockPos pos = decodeBlockPos(intPos).add(posTNT);
				if(Math.random() > 0.75f && level.getBlockState(pos).isAir() && level.getBlockState(pos.down()).isOpaqueFullCube()) {
...
/**
		...
		for(Entity entity : entities) {
			if(!entity.isImmuneToExplosion(null)) {
				...
				if(distance <= 1f) {
					...
					if(damageEntities) {
						entity.damage((ServerWorld)level, damageSource, (damage * damage + damage) / 2f * 7 * size + 1f);
					}
	...
	public java.util.Map<PlayerEntity, Vec3d> getAffectedPlayers() {
		return affectedPlayers;
	}
	
	public double getExposure(Vec3d source, Entity entity) {
			...
			for (double k = 0.0; k <= 1.0; k += d) {
				for (double l = 0.0; l <= 1.0; l += e) {
					for (double m = 0.0; m <= 1.0; m += f) {
						...
						if (level.raycast(new net.minecraft.world.RaycastContext(vec3d, source, net.minecraft.world.RaycastContext.ShapeType.COLLIDER, net.minecraft.world.RaycastContext.FluidHandling.NONE, entity)).getType() == net.minecraft.util.hit.HitResult.Type.MISS) {
							++i;
						}
